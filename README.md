...
比赛奖品兑换平台轻量版。管理员用单一环境变量密码管理比赛、奖品池、获奖人、兑换记录、采购报销和通知；获奖人用一次性兑换码在 quota 与实时库存范围内一次选择多件奖品，并到固定地点自提。

## 核心功能

### 全局奖品池

所有比赛共享同一个奖品池，不再按比赛隔离。管理员在 `/admin/prizes` 统一管理奖品库存、价格、标签和上下架状态。

每个比赛可以**主动选择**从全局奖品池中哪些奖品对该比赛可用：
- 在 `/admin/events/:id` 比赛详情页的"奖品"标签中，管理员可以看到所有全局奖品，并勾选哪些对本比赛可用
- 获奖人兑换时只能看到该比赛选中的奖品
- 迁移时，现有奖品的比赛归属关系会自动保存到 `event_prize_availability` 表，确保历史数据平滑过渡

### 奖项名称

每个获奖人可以配置一个奖项名称（如"一等奖"、"最佳创意奖"），该名称会：
- 显示在获奖人列表中
- 包含在兑换码发放邮件的模板变量 `{{award_name}}` 中
- 导出到获奖人 CSV/Excel 的 `award_name` 列
- 导出到兑换记录 CSV/Excel 的 `award_name` 列

### 采购报销

管理员在 `/admin/purchases` 创建采购订单，流程包括：
1. **创建采购单**：从奖品池选择奖品并指定数量，系统自动快照奖品名称和单价
2. **上传附件**：上传交易截图（JPEG/PNG/WebP）和发票 PDF 作为报销凭证
3. **标记报销**：必须同时包含交易截图和发票才能标记为已报销
4. **打包下载**：下载包含 manifest.xlsx 和所有附件的 ZIP 包
5. **导出列表**：导出采购单列表为 CSV/Excel
