...
watch(groups, (next) => {
  for (const group of next) if (!(groupKey(group.tag) in openGroups)) openGroups[groupKey(group.tag)] = true
})
const allCollapsed = computed(
  () => groups.value.length > 0 && groups.value.every((group) => !openGroups[groupKey(group.tag)]),
)
function toggleAllGroups() {
  // When everything is collapsed, expand all; otherwise collapse all.
  const next = allCollapsed.value
  for (const group of groups.value) openGroups[groupKey(group.tag)] = next
}
...
async function continueToConfirm() {
  if (redemption.customPrize || (itemCount.value > 0 && remaining.value >= 0)) await router.push('/redeem/confirm')
}
    ...
    <template v-else-if="redemption.context">
      <header class="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 class="mt-1 text-2xl font-bold sm:text-3xl">{{ redemption.context.winner.name }}，请选择奖品</h1>
        ...
        <div class="card flex w-full justify-around py-4 text-center">
          <div>
            ...
            ><span class="text-xs text-slate-500 dark:text-slate-400">总 quota</span>
          </div>
          <div>
            <strong class="block text-xl text-blue-600 dark:text-blue-400">{{ used }}</strong
            ><span class="text-xs text-slate-500 dark:text-slate-400">已使用</span>
          </div>
          <div>
            <strong class="block text-xl text-emerald-600 dark:text-emerald-400">{{ remaining }}</strong
            ><span class="text-xs text-slate-500 dark:text-slate-400">剩余</span>
          </div>
        </div>
      </header>
      <div
        ...
        >
          <CollapsibleTrigger class="card flex w-full items-center justify-between gap-3 p-4 text-left">
            <span class="flex items-baseline gap-2">
              ...
              <span class="text-xs text-slate-500 dark:text-slate-400">{{ group.prizes.length }} 件奖品</span>
            </span>
            <svg
            ...
            <section class="mt-3 grid gap-3 sm:mt-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-3">
              <article v-for="prize in group.prizes" :key="prize.id" class="card overflow-hidden p-0">
                ...
                <div class="p-4 sm:p-5">
                  <div class="flex flex-wrap items-start justify-between gap-2 sm:flex-nowrap sm:gap-3">
        ...
        <template v-if="redemption.customPrize">
          <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="min-w-0">
              ...
              <strong class="mt-1 block break-words text-lg">{{ redemption.customPrize.name }}</strong>
              <p
              ...
              >
              <p v-if="redemption.customPrize.note" class="mt-1 text-sm text-slate-500 dark:text-slate-400">
      ...
      <div class="fixed bottom-4 left-4 right-4 z-20 sm:sticky sm:bottom-4 sm:left-auto sm:right-auto sm:mt-8">
        ...
        >
          <div class="flex items-center justify-between gap-3 sm:block">
            <template v-if="redemption.customPrize">
              <strong>自定义奖品</strong><span class="text-sm text-slate-300">{{ redemption.customPrize.name }}</span>
            </template>
            <template v-else>
              <strong>{{ itemCount }} 件奖品</strong
              ><span class="text-sm" :class="remaining < 0 ? 'text-red-300' : 'text-slate-300'"
                >消耗 {{ used }}，{{ remaining < 0 ? `超出 ${Math.abs(remaining)}` : `剩余 ${remaining}` }}</span
              >
            </template>
          </div>
          <button
