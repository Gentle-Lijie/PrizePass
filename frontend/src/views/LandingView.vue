...
const features = computed(() => (role.value === 'winner' ? winnerFeatures : organizerFeatures))
...
const used = computed(() => demoPrizes.reduce((sum, p) => sum + p.value * (cart.value[p.id] ?? 0), 0))
const remaining = computed(() => QUOTA - used.value)
const progress = computed(() => Math.min(100, (used.value / QUOTA) * 100))
const pickedCount = computed(() => Object.values(cart.value).reduce((s, n) => s + n, 0))
  ...
  <div class="min-h-screen bg-canvas text-ink">
    ...
    >
      <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6">
        <a href="/" class="flex items-center gap-2.5">
          ...
          <span class="text-sm font-semibold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400"
            >PrizePass</span
          >
        </a>
    ...
    >
      <div class="mx-auto grid max-w-6xl items-center gap-10 px-4 py-12 sm:px-6 sm:py-20 lg:grid-cols-2 lg:py-20">
        <div v-reveal>
          ...
          <h1 class="mt-3 text-5xl font-bold leading-[1.08] tracking-tight sm:text-6xl">奖品，你自己挑。</h1>
          ...
          <dl class="mt-10 grid max-w-md grid-cols-3 gap-4 border-t border-slate-200 pt-6 dark:border-slate-700">
            <div>
              <dt class="text-2xl font-bold text-ink">7</dt>
              <dd class="mt-1 text-xs text-slate-500 dark:text-slate-400">种通知模板</dd>
            </div>
            <div>
              <dt class="text-2xl font-bold text-ink">3+</dt>
              <dd class="mt-1 text-xs text-slate-500 dark:text-slate-400">种通知渠道</dd>
            </div>
            <div>
              <dt class="text-2xl font-bold text-ink">0</dt>
              <dd class="mt-1 text-xs text-slate-500 dark:text-slate-400">Cookie / Token</dd>
            </div>
          </dl>
        </div>
        ...
        >
          <div class="flex items-center justify-between">
            <p class="text-sm font-semibold uppercase tracking-[0.18em] text-blue-400">体验兑换</p>
            <span class="rounded-full bg-emerald-400/15 px-2.5 py-1 text-xs font-medium text-emerald-300"
              >实时库存</span
            >
          </div>
          <div class="mt-5 min-h-0 flex-1">
            <div v-if="demoSubmitted" class="grid h-full place-items-center">
              <div class="text-center">
                ...
                <h3 class="mt-5 text-xl font-semibold text-white">已提交待备货</h3>
            ...
            <div v-else>
              ...
              <div v-if="demoWishSubmitted" class="grid h-full place-items-center">
                <div class="text-center">
                  ...
                  <h3 class="mt-5 text-xl font-semibold text-white">申请已提交</h3>
                  <p class="mt-2 text-sm text-slate-400">等待管理员确认，采纳或驳回都会邮件通知你</p>
                  <button
                    type="button"
                    class="mt-8 rounded-xl bg-white px-8 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100"
                    @click="backToList"
              ...
              <form v-else-if="demoCustom" class="flex h-full flex-col" @submit.prevent="submitWishDemo">
                <p class="text-sm text-slate-400">没有想要的？自己填一个。</p>
                <div class="mt-4 grid gap-3">
                  <input
                    v-model="customDemo.name"
                    ...
                    maxlength="200"
                    placeholder="奖品名称，如 小米手环 9"
                  />
                  <input
                    v-model="customDemo.url"
                    class="w-full rounded-xl bg-slate-900 px-3.5 py-2.5 text-sm text-white outline-none ring-1 ring-slate-700 placeholder:text-slate-500 focus:ring-blue-500"
                    maxlength="2000"
                    placeholder="商品链接（选填）"
                  />
                  <input
                    v-model="customDemo.price"
                    class="w-full rounded-xl bg-slate-900 px-3.5 py-2.5 text-sm text-white outline-none ring-1 ring-slate-700 placeholder:text-slate-500 focus:ring-blue-500"
                    inputmode="decimal"
                    placeholder="期望价格（元，选填）"
                  />
                </div>
                <p class="mt-3 text-xs text-amber-400/90">提交前会确认是否可以开票；被拒绝时兑换码自动恢复。</p>
              ...
              <template v-else>
                <div class="flex items-end justify-between text-sm">
                  ...
                  <span class="font-mono"
                    ><span class="text-lg font-bold text-white">{{ used }}</span
                    ><span class="text-slate-500"> / {{ QUOTA }}</span></span
                  >
                </div>
                <div class="mt-2 h-2 overflow-hidden rounded-full bg-slate-800">
                  <div
                    class="h-full rounded-full bg-gradient-to-r from-blue-500 to-indigo-400 transition-all duration-300"
                    :style="{ width: `${progress}%` }"
                  />
                </div>
                <p class="mt-2 text-right text-xs text-slate-500">剩余 {{ remaining }}</p>
                <ul class="mt-5 space-y-2">
                  <li v-for="p in demoPrizes" :key="p.id" class="flex items-center gap-3 rounded-xl bg-slate-900 p-3">
                    <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-slate-800 text-lg">{{
                      p.emoji
                    }}</span>
                    ...
                    <div class="flex items-center gap-1.5">
                      ...
                      <span class="w-5 text-center font-mono text-sm">{{ cart[p.id] ?? 0 }}</span>
                      <button
    ...
    <section class="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
      <div v-reveal class="mx-auto max-w-2xl text-center">
        <h2 class="text-3xl font-bold tracking-tight sm:text-4xl">为两种角色而生</h2>
        <p class="mt-3 text-slate-600 dark:text-slate-300">无论是领奖人还是组织者，都能在两步之内开始。</p>
      </div>
    ...
    <section class="border-y border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
        <div v-reveal class="max-w-2xl">
          <h2 class="text-3xl font-bold tracking-tight sm:text-4xl">从建赛到领取，一条直线</h2>
          <p class="mt-3 text-slate-600 dark:text-slate-300">五步走完整个流程，每一步的状态都可追溯、可回滚。</p>
        </div>
    ...
    <section class="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
      <div class="grid items-center gap-10 lg:grid-cols-2">
        <div v-reveal>
          ...
          <h2 class="mt-4 text-3xl font-bold tracking-tight sm:text-4xl">每个场景，独立路由</h2>
          ...
          <ul class="mt-6 space-y-2.5 text-sm text-slate-700 dark:text-slate-300">
            <li class="flex items-center gap-2">
              <span class="text-emerald-600">✓</span> 失败按 1 分钟、5 分钟自动重试
            </li>
            <li class="flex items-center gap-2"><span class="text-emerald-600">✓</span> 纯文本与 HTML 双模板</li>
            <li class="flex items-center gap-2"><span class="text-emerald-600">✓</span> 通知任务监控与手工重试</li>
          </ul>
        </div>
        <div
          v-reveal
        ...
        >
          <p class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-slate-500">场景路由示意</p>
          <div class="mt-4 space-y-3">
            <div
              ...
              :key="row.s"
              class="flex items-center justify-between rounded-xl bg-canvas px-4 py-3"
            >
              <span class="text-sm font-medium">{{ row.s }}</span>
              <span class="text-xs text-slate-500 dark:text-slate-400">{{ row.c }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="bg-slate-950">
      <div class="mx-auto max-w-4xl px-4 py-16 text-center sm:px-6 sm:py-24">
        <h2 v-reveal class="text-3xl font-bold tracking-tight text-white sm:text-4xl">准备好开始了吗？</h2>
    ...
    <footer class="border-t border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div class="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 py-4 sm:flex-row sm:px-6">
        <div class="flex items-center gap-2.5">
          <span class="grid h-7 w-7 place-items-center rounded-md bg-accent text-sm font-bold text-white">P</span>
          <span class="text-sm font-semibold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400"
            >PrizePass</span
          >
        </div>
        ...
        >
          <a
            ...
            class="hover:text-ink dark:hover:text-slate-200"
            >沪ICP备2025155706号-1</a
          >
          <a
