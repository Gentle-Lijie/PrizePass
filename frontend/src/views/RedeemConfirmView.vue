...
const used = computed(() => selected.value.reduce((sum, prize) => sum + prize.redeem_value * redemption.quantities[prize.id]!, 0))
  ...
  try {
    redemption.success = await api<RedemptionSuccess>('/api/public/redemptions', {
      ...
      body: JSON.stringify({
        ...
        note: form.note || null,
        items: selected.value.map((prize) => ({ prize_id: prize.id, quantity: redemption.quantities[prize.id] })),
      }),
    })
    auth.clearRedemptionCode()
    await router.replace('/redeem/success')
  } catch (caught) { error.value = caught instanceof Error ? caught.message : '提交失败，请重试' }
  finally { busy.value = false }
}

onMounted(async () => {
  if (!auth.redemptionCode || !redemption.context || selected.value.length === 0) await router.replace('/redeem')
})
</script>
