...
const used = computed(() =>
  selected.value.reduce((sum, prize) => sum + prize.redeem_value * redemption.quantities[prize.id]!, 0),
)
      ...
      body: JSON.stringify({
        ...
        ...(custom
          ? {
              ...
              custom_price: custom.priceYuan ? Math.round(Number(custom.priceYuan) * 100) : null,
            }
          : {
...
onMounted(async () => {
  if (!auth.redemptionCode || !redemption.context || (selected.value.length === 0 && !redemption.customPrize))
...
<template>
  <main class="mx-auto min-h-screen max-w-3xl p-6 pb-18 md:p-10 md:pb-32">
    <button class="text-sm text-blue-600 dark:text-blue-400" @click="router.back()">← 返回选择奖品</button>
    <h1 class="mt-4 text-3xl font-bold">确认兑换</h1>
    <form v-if="redemption.context" class="mt-6 grid gap-6" @submit.prevent="requestSubmit">
      <section class="card">
        ...
        <template v-if="isCustom && redemption.customPrize">
          <div class="mt-4 border-t border-slate-200 pt-4 text-sm dark:border-slate-700">
            <p>
              ...
              <strong v-if="redemption.customPrize.priceYuan" class="ml-2 text-blue-600 dark:text-blue-400"
            ...
            >
            <p v-if="redemption.customPrize.note" class="mt-2 text-slate-500 dark:text-slate-400">
        ...
        <template v-else>
          ...
          >
            <span class="break-words">{{ prize.name }} × {{ redemption.quantities[prize.id] }}</span
            ><strong class="shrink-0">{{ prize.redeem_value * redemption.quantities[prize.id]! }} 额度</strong>
          </div>
          <div
          ...
          >
            ...
            ><span class="text-emerald-600 dark:text-emerald-400">剩余 {{ remaining }}</span>
          </div>
        </template>
      </section>
      <section class="card">
        ...
        <div class="mt-4 grid gap-4">
          <label class="text-sm font-medium"
            >姓名 <input v-model="form.contact_name" class="field mt-1" maxlength="100" required
          /></label>
          <label class="text-sm font-medium"
            >手机号
            <input
          ...
          <label class="text-sm font-medium"
            >备注（可选）
            <textarea v-model="form.note" class="field mt-1" maxlength="500" rows="3" />
          </label>
        </div>
      </section>
    ...
    >
      <div class="card w-full max-w-md">
        ...
        <div class="mt-6 flex flex-col gap-2 sm:flex-row">
          <button class="btn-primary flex-1" :disabled="busy" @click="submit">
            {{ busy ? '提交中…' : '确认可以开票，提交' }}
          </button>
          <button class="btn-secondary flex-1" type="button" @click="showInvoiceConfirm = false">返回检查</button>
        </div>
      </div>
    </div>
  </main>
