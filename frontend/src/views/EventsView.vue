...
<template>
  <main class="mx-auto max-w-6xl p-6 md:p-10">
    <header class="mb-8 flex flex-wrap items-center justify-between gap-4">
      <div>
        <p class="text-sm font-semibold text-blue-600 dark:text-blue-400">PrizePass 后台</p>
        <h1 class="mt-1 text-3xl font-bold">比赛</h1>
      </div>
      <div class="flex flex-wrap gap-2">
        <RouterLink class="btn-secondary" to="/admin/prizes">全局奖品池</RouterLink>
        <RouterLink class="btn-secondary" to="/admin/purchases">采购报销</RouterLink>
        <button class="btn-secondary" type="button" :disabled="loading" @click="refreshList">刷新状态</button
        ><RouterLink class="btn-secondary" to="/admin/settings/notifications">通知设置</RouterLink
        ><button class="btn-primary" @click="showCreate = true">新建比赛</button>
      </div>
    </header>
    ...
    <section v-else-if="events.length === 0" class="card text-center">
      ...
      <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">创建第一场比赛并配置自提信息与奖品。</p>
    </section>
    <section v-else class="grid gap-4">
      ...
      >
        <div>
          ...
          <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">截止 {{ localTime(event.redemption_deadline) }}</p>
        </div>
    ...
    >
      <form class="card max-h-[90vh] w-full max-w-xl overflow-auto" @submit.prevent="createEvent">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-bold">新建比赛</h2>
          <button type="button" class="text-slate-500 dark:text-slate-400" @click="showCreate = false">关闭</button>
        </div>
        <div class="mt-5 grid gap-4">
          <label class="text-sm font-medium"
            >名称<input v-model="form.name" class="field mt-1" maxlength="200" required
          /></label>
          <label class="text-sm font-medium"
            >说明<textarea v-model="form.description" class="field mt-1" rows="3" />
          </label>
          <label class="text-sm font-medium"
            >兑换截止时间<input v-model="form.redemption_deadline" class="field mt-1" type="datetime-local" required
          /></label>
          <label class="text-sm font-medium"
            >自提地点<textarea v-model="form.pickup_location" class="field mt-1" rows="2" required />
          </label>
          <label class="text-sm font-medium"
            >自提说明<textarea v-model="form.pickup_instructions" class="field mt-1" rows="3" required />
          </label>
          <label class="text-sm font-medium"
            >比赛总预算（元）<input v-model="budgetYuan" class="field mt-1" inputmode="decimal" required
          /></label>
        </div>
