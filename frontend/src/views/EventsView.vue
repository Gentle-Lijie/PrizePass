...
<template>
  <main class="mx-auto max-w-6xl p-6 md:p-10">
    ...
    <section v-if="loading" class="card text-slate-500">正在加载比赛…</section>
    <section v-else-if="events.length === 0" class="card text-center">
      <h2 class="text-lg font-semibold">还没有比赛</h2>
      <p class="mt-2 text-sm text-slate-500">创建第一场比赛并配置自提信息与奖品。</p>
    </section>
      ...
      >
        <div>
          <div class="flex items-center gap-3">
            ...
            <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium">{{ statusLabel(event.status) }}</span>
          </div>
    ...
    <div v-if="showCreate" class="fixed inset-0 z-20 grid place-items-center bg-slate-950/40 p-4" @click.self="showCreate = false">
      <form class="card max-h-[90vh] w-full max-w-xl overflow-auto" @submit.prevent="createEvent">
        <div class="flex items-center justify-between"><h2 class="text-xl font-bold">新建比赛</h2><button type="button" class="text-slate-500" @click="showCreate = false">关闭</button></div>
        <div class="mt-5 grid gap-4">
          <label class="text-sm font-medium">名称<input v-model="form.name" class="field mt-1" maxlength="200" required /></label>
          <label class="text-sm font-medium">说明<textarea v-model="form.description" class="field mt-1" rows="3" /></label>
