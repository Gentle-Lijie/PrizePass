...
function openHtmlPreview(template: NotificationTemplateRecord) {
  ...
  const html = template.html_template.replace(
  ...
  preview.value = {
    title: eventLabels[template.event_type] ?? template.event_type,
    html,
  }
}
...
<template>
  <main class="mx-auto max-w-6xl p-6 md:p-10">
    ...
    <section class="card mt-6">
      ...
      <div class="mt-4 flex flex-wrap gap-3">
        <span
          ...
          "
          >SMTP {{ configuration.smtp ? '已配置' : '未配置' }}</span
        >
        <span
          class="rounded-full px-3 py-1 text-sm"
          :class="
        ...
        <span
          ...
          :class="
            configuration.webhook
              ...
              : 'bg-slate-100 text-slate-500 dark:text-slate-400 dark:bg-slate-800 dark:text-slate-400'
          "
          >Webhook {{ configuration.webhook ? '已配置' : '未配置' }}</span
        >
        <span
          class="rounded-full px-3 py-1 text-sm"
          :class="
            configuration.email_poster
              ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300'
              : 'bg-slate-100 text-slate-500 dark:text-slate-400 dark:bg-slate-800 dark:text-slate-400'
          "
      ...
      <div class="mt-5 flex flex-wrap items-end gap-3">
        ...
        ><button
          class="btn-secondary w-full sm:w-auto"
          :disabled="busy || !email || !configuration.email_poster"
    ...
    <section class="mt-8">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
            可为同一场景选择多个渠道和收件对象；设置仅影响之后创建的任务。
          </p>
        </div>
        ...
        <table class="w-full min-w-[920px] text-left text-sm">
          ...
          <tbody>
            <tr
              ...
              :key="route.event_type"
              class="border-t border-slate-200 dark:border-slate-700"
            >
              <th class="p-4 font-medium">
                {{ eventLabels[route.event_type] }}
              </th>
              <td class="p-4 text-center">
                <input
                  v-model="route.smtp_winner"
                  class="h-4 w-4 accent-blue-600"
                  type="checkbox"
                  :aria-label="`${eventLabels[route.event_type]} SMTP 发给获奖人`"
                />
              </td>
              <td class="p-4 text-center">
                <input
                  v-model="route.smtp_operations"
                  class="h-4 w-4 accent-blue-600"
                  type="checkbox"
                  :aria-label="`${eventLabels[route.event_type]} SMTP 发给运营邮箱`"
                />
              </td>
              <td class="p-4 text-center">
                <input
                  v-model="route.email_poster_winner"
                  class="h-4 w-4 accent-blue-600"
                  type="checkbox"
                  :aria-label="`${eventLabels[route.event_type]} email-poster 发给获奖人`"
                />
              </td>
              <td class="p-4 text-center">
                <input
                  v-model="route.email_poster_operations"
                  class="h-4 w-4 accent-blue-600"
                  type="checkbox"
                  :aria-label="`${eventLabels[route.event_type]} email-poster 发给运营邮箱`"
                />
              </td>
              <td class="p-4 text-center">
                <input
                  v-model="route.webhook"
                  class="h-4 w-4 accent-blue-600"
                  type="checkbox"
                  :aria-label="`${eventLabels[route.event_type]} 发送 Webhook`"
                />
              </td>
            </tr>
          </tbody>
      ...
      <p class="mt-3 text-xs leading-5 text-slate-500 dark:text-slate-400">
        “运营邮箱”来自 <code>NOTIFICATION_EMAIL</code>；Webhook 发送到
        <code>WEBHOOK_URL</code>。未配置的环境目标会导致对应任务发送失败。
      </p>
    </section>
        ...
        <article v-for="template in templates" :key="template.event_type" class="card">
          ...
          <div class="mt-4 flex items-center justify-between gap-3">
            ...
            <button
              ...
              type="button"
              :disabled="!template.html_template?.trim()"
              @click="openHtmlPreview(template)"
            >
              预览 HTML
            </button>
          </div>
          <textarea
            v-model="template.html_template"
            class="field mt-2 min-h-52 font-mono text-sm"
            maxlength="50000"
            placeholder="留空则仅发送纯文本正文"
          />
          <p class="mt-3 text-xs leading-6 text-slate-500 dark:text-slate-400">
            可用变量：<code
              v-for="variable in template.allowed_variables"
              :key="variable"
              class="mr-2 rounded bg-slate-100 px-1.5 py-1 dark:bg-slate-800"
              >{{ variableLabel(variable) }}</code
            >
          </p>
        </article>
      </div>
    ...
    <Teleport to="body">
      ...
      >
        <section
          class="flex h-[min(90vh,900px)] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-900"
          role="dialog"
          aria-modal="true"
          :aria-label="`${preview.title} HTML 预览`"
        >
          <header
            class="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4 dark:border-slate-700"
          >
            <div>
