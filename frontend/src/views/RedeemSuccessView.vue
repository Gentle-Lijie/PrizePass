...
<template>
  <main class="min-h-screen bg-emerald-50 px-4 py-6 dark:bg-emerald-950/20 sm:grid sm:place-items-center sm:p-6">
    <section v-if="redemption.success" class="card mx-auto w-full max-w-lg p-5 text-center sm:p-6">
      ...
      <div class="mt-5 min-w-0 rounded-xl bg-slate-950 p-4 text-white sm:mt-6 sm:p-5">
        <p class="text-xs text-slate-400">兑换单号</p>
        <strong class="mt-2 block break-all font-mono text-base leading-7 tracking-wide sm:text-xl sm:tracking-wider">{{
          redemption.success.order_no
        }}</strong>
      ...
      <button class="btn-secondary mt-6 w-full" @click="router.push('/')">返回首页</button>
    </section>
  </main>
</template>
