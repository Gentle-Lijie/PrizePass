...
import { eventTabContextKey, type EventTabContext } from '@/components/event/eventContext'
...
const prizeSummary = ref<EventPrizeSummary>({
...
async function load() {
  ...
  try {
    const [eventData, prizeData, summaryData, winnerData, redemptionData] = await Promise.all([
      ...
      api<EventPrizeSummary>(`/api/admin/events/${eventId}/prizes/summary`),
...
<template>
  <main class="mx-auto max-w-7xl p-6 md:p-10">
    <div class="flex flex-wrap justify-between gap-3">
      <RouterLink to="/admin/events" class="text-sm text-blue-600 dark:text-blue-400 hover:underline"
        >← 返回比赛列表</RouterLink
      ><RouterLink to="/admin/settings/notifications" class="text-sm text-blue-600 dark:text-blue-400 hover:underline"
        >通知设置</RouterLink
      >
    </div>
    <header v-if="event" class="mt-4 flex flex-wrap items-end justify-between gap-4">
      <div>
        <div class="flex items-center gap-3">
          ...
          <span class="rounded-full bg-slate-100 px-3 py-1 text-xs dark:bg-slate-800 dark:text-slate-300">{{
      ...
      <button class="btn-secondary" type="button" :disabled="busy" @click="refreshForm">
        {{ busy ? '刷新中…' : '刷新表单' }}
      </button>
    </header>
