...
async function refreshForm() {
  ...
  try {
    ...
    if (selectedRedemption.value)
      await openRedemption(selectedRedemption.value.id)
...
<template>
  <main class="mx-auto max-w-7xl p-6 md:p-10">
    ...
    <section v-if="tab === 'prizes'" class="mt-6">
      <div class="mb-5 grid gap-4 md:grid-cols-3">
        ...
        <article class="card">
          ...
          <strong
            class="mt-2 block text-2xl text-emerald-600 dark:text-emerald-400"
      ...
      <div v-if="importPreview" class="card mt-4">
        ...
        <div class="mt-4 max-h-48 overflow-auto">
          <table class="w-full text-left text-sm">
            ...
            <tbody>
              <tr
                v-for="(row, index) in importPreview.rows"
                :key="index"
    ...
    >
      <form
        class="card max-h-[92vh] w-full max-w-xl overflow-auto"
        ...
        <div class="mt-5 grid gap-4">
          ...
          <fieldset>
            ...
            <div class="mt-2 flex gap-4 text-sm">
              <label
                ><input v-model="imageMode" type="radio" value="url" /> HTTPS
                外链</label
              ><label
            ...
            <div v-else class="mt-2">
              ...
              <p
                ...
                class="mt-2 break-all text-xs text-slate-500 dark:text-slate-400"
              >
                {{ prizeForm.image }}
              </p>
            </div>
          ...
          <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
            <label class="text-sm font-medium"
              >真实采购单价（元）<input
                v-model="prizeForm.realValueYuan"
                class="field mt-1"
                inputmode="decimal"
                required /></label
            ><label class="text-sm font-medium"
              >用户展示价格（元）<input
                v-model="prizeForm.purchaseValueYuan"
                class="field mt-1"
                inputmode="decimal"
                required /></label
            ><label class="text-sm font-medium"
              >抵扣额度<input
                v-model.number="prizeForm.redeem_value"
                class="field mt-1"
                type="number"
                min="1"
                step="1"
            ...
            ><label class="text-sm font-medium"
              >库存<input
                v-model.number="prizeForm.stock"
                class="field mt-1"
                type="number"
                step="1"
                required
