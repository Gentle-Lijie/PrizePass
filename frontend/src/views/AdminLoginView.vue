...
<template>
  <main class="grid min-h-screen place-items-center p-6">
    <form class="card w-full max-w-sm" @submit.prevent="enter">
      ...
      <p class="mt-2 text-sm text-slate-500">密码仅保存在当前页面内存，刷新后需要重新输入。</p>
      <label class="mt-6 block text-sm font-medium" for="password">管理员密码</label>
      <input id="password" v-model="password" class="field mt-2" type="password" required autocomplete="current-password" />
