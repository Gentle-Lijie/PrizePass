...
def test_event_and_six_field_prize_crud() -> None:
    ...
    response = client.post(
        f"/api/admin/prizes", headers=ADMIN, json=prize_payload()
    )
    assert response.status_code == 201, response.text
...
def test_prize_rejects_extra_fields_and_non_https_image() -> None:
    ...
    assert client.post(f"/api/admin/prizes", headers=ADMIN, json=bad).status_code == 422
...
def test_csv_import_is_atomic_and_export_has_money_format() -> None:
    ...
    response = client.post(
        f"/api/admin/prizes/import/validate",
        headers=ADMIN,
    ...
    response = client.post(
        f"/api/admin/prizes/import/confirm",
        headers=ADMIN,
        files={"file": ("prizes.csv", valid, "text/csv")},
    )
    ...
    export = client.get(f"/api/admin/prizes/export?format=csv", headers=ADMIN)
    ...
    with Session(test_engine) as session:
        assert session.scalar(select(func.count(Prize.id))) == 2
...
def test_prize_is_active_and_admin_list_orders_by_tag() -> None:
    ...
    listed = client.get(f"/api/admin/prizes", headers=ADMIN).json()
    ...
    assert client.post(f"/api/admin/prizes", headers=ADMIN, json=off_shelf).status_code == 201
    listed = client.get(f"/api/admin/prizes", headers=ADMIN).json()
...
def test_prize_batch_tag_and_stock_and_delete() -> None:
    ...
    assert client.post(
        f"/api/admin/prizes/batch-stock",
        headers=ADMIN,
    ...
    with Session(test_engine) as session:
        assert session.scalar(select(func.count(Prize.id))) == 1
...
def test_csv_import_with_tag_column() -> None:
    ...
    assert response.status_code == 201, response.text
    prizes = client.get(f"/api/admin/prizes", headers=ADMIN).json()
...
def test_xlsx_import_and_export_match_csv() -> None:
    ...
    assert response.status_code == 201, response.text
    exported = client.get(f"/api/admin/prizes/export?format=xlsx", headers=ADMIN)
